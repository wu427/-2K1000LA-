...
class VideoServer1(QtCore.QObject):
    ...
    update_speech = QtCore.pyqtSignal(str)  # 新增语音识别信号
    def __init__(self, ip, port):
        ...
        # 语音识别初始化
        self.speech_model = Model("vosk-model-cn-0.22")  # 中文模型路径
    ...
    def start_speech_recognition(self):
        ...
        if not self.is_speech_running:
            ...
            self.speech_recognizer = KaldiRecognizer(self.speech_model, 16000)
    ...
    #             if text:
    #                 print(f"语音识别结果: {text}")
    #                 self.update_speech.emit(text)  # 发送识别结果信号
    #
    #     stream.stop_stream()
    #     stream.close()
    #     p.terminate()
        ...
        while self.is_speech_running:
            ...
            if self.speech_recognizer.AcceptWaveform(data):
                ...
                if text:
                    ...
                    class_names = []
                    if "火" in text:
                        class_names.append("3")  # 火用1表示
                    if "烟" in text or "烟雾" in text:
                        class_names.append("4")  # 烟用0表示

                    # 如果有检测到关键词，发送信号
                    if class_names:
                        self.update_classes.emit(class_names)
                        print(f"语音：{class_names}")
                    # 同时发送原始文本(可选)
                    self.update_speech.emit(text)
    ...
    def start(self):
        ...
        self.start_speech_recognition()  # 同时启动语音识别
                ...
                if data_type == b'a':  # 视频数据
                    ...
                    if class_data:
                        ...
                        try:
                            ...
                            # # 解析并标准化类别数据
                            # raw_class_names = class_data.split(',')
                            # standardized_classes = []
                            #
                            # for name in raw_class_names:
                            #     name_lower = name.lower().strip()
                            #     if "火" in name_lower:
                            #         standardized_classes.append("-1")  # 火用1表示
                            #     elif "烟" in name_lower or "烟雾" in name_lower:
                            #         standardized_classes.append("-2")  # 烟用0表示
                            #     else:
                            #         standardized_classes.append(name)  # 其他类别保持原样
                            # print(standardized_classes)
                            # self.update_classes.emit(standardized_classes)
...
class APP1(object):
    ...
    def __init__(self, ip, port):
        ...
        self.continuous_alarm = False  # 新增持续报警标志
        self.alarm_timer = QtCore.QTimer()  # 报警检查定时器
        self.alarm_timer.timeout.connect(self.check_alarm_status)
        self.textEdit_3.mousePressEvent = self.handle_alarm_stop  # 绑定点击事件
    def setupUi(self):
    ...
    #         self.movie.start()
    def handle_alarm_stop(self, event):
        """处理手动停止报警"""
        self.continuous_alarm = False
        self.stop_alarm_sound1()
        self.update_gif_display("安全.gif")
        QtWidgets.QTextEdit.mousePressEvent(self.textEdit_3, event)  # 保持原有功能
    ...
    def start_continuous_alarm(self):
        """启动持续报警"""
        self.continuous_alarm = True
        if not self.is_alarm_playing1:
            print(1111111111)
            self.play_alarm_sound1()
        self.alarm_timer.start(10000)  # 每秒检查一次
    ...
    def check_alarm_status(self):
        ...
        if self.continuous_alarm:
            ...
            self.stop_alarm_sound1()
    def play_alarm_sound1(self):
        """播放警报音"""
        try:
            self.player1.setMedia(QtMultimedia.QMediaContent(QtCore.QUrl.fromLocalFile("alarm.wav")))
            self.player1.play()
            self.is_alarm_playing1 = True
        except Exception as e:
            print(f"播放警报音失败: {e}")
    def stop_alarm_sound1(self):
        """停止警报音"""
        self.player1.stop()
        self.is_alarm_playing1 = False
    ...
    #
    #     if '4' in classes:
    #         self.detected_classes.add("烟（音频）")
    #         current_classes.append("烟（音频）")
    #         self.last_detection_time = QtCore.QDateTime.currentDateTime()
    #         self.play_alarm_sound1()
    #
    #     elif '0' in classes:
    ...
    #
    #     if '3' in classes:
    #         self.detected_classes.add("火（音频）")
    #         current_classes.append("火（音频）")
    #         fire_detected = True
    #         self.last_detection_time = QtCore.QDateTime.currentDateTime()
    #         self.play_alarm_sound1()
    #     elif '1' in classes:
    ...
    def update_classes(self, classes):
        ...
        # 处理检测结果
        if '4' in classes or '0' in classes:
            cls_name = "烟(音频)" if '4' in classes else "烟"
            self.detected_classes.add(cls_name)
            current_classes.append(cls_name)
            self.last_detection_time = QtCore.QDateTime.currentDateTime()

        if '3' in classes or '1' in classes:
            cls_name = "火(音频)" if '3' in classes else "火"
            self.detected_classes.add(cls_name)
            current_classes.append(cls_name)
            fire_detected = True
            self.last_detection_time = QtCore.QDateTime.currentDateTime()
            ...
            # 保存火灾画面
            if fire_detected and hasattr(self, 'latest_image'):
                saved_path = self.save_fire_image(self.latest_image)
                if saved_path:
                    self.textEdit.append(f"火灾画面已保存: {saved_path}")

        # 更新UI状态
        self.update_ui_status(current_classes, fire_detected)

        # 语音检测(3/4)触发持续报警
        if '3' in classes or '4' in classes:
            self.start_continuous_alarm()

    def update_ui_status(self, current_classes, fire_detected):
        """更新UI状态"""
        self.textEdit_3.clear()

        if current_classes:
            status = "检测到：" + "、".join(current_classes)
            if self.continuous_alarm:
                status += " (点击此处解除警报)"
            self.textEdit_3.append(status)

            gif_path = "危险.gif"
            if fire_detected and not self.is_alarm_playing:
                self.play_alarm_sound()
        else:
            status = "未检测出火灾"
            gif_path = "安全.gif"
        ...
        self.update_gif_display(gif_path)
